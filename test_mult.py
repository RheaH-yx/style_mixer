...
parser.add_argument('--output', type=str, default='./wikiart/generated/Abstract_Expressionism',
...
network.load_state_dict(torch.load(config['mw'], map_location=torch.device('cpu')))
network.eval()
network.to(device)
...
for content_path in content_paths:
    ...
    output_name = '{:s}/{:s}{:s}'.format(
        out, config['pre_def']+'_'+splitext(basename(config['style']))[0],
        config['save_ext']
    )
